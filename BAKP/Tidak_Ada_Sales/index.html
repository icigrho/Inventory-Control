...
// ---------------------------------------------------------
// 3. DAFTAR PILIHAN QUERY (TIDAK ADA SALES)
// ---------------------------------------------------------
$queries = [
    'tidak_ada_sales' => [
...
$active_query_config = $queries['tidak_ada_sales'];
...
if ($action == 'export_excel') {
    $filename = "Laporan_TIDAK_ADA_SALES_" . date('Ymd_His');
            ...
            if ($result) {
                ...
                while ($row = pg_fetch_assoc($result)) {
                    ...
                    foreach ($row as $key => $value) {
                        ...
                        if (in_array($field_name, ['plu', 'div', 'dept', 'katb', 'tag', 'tag_omi'])) {
                            echo "<td style='mso-number-format:\"\\@\";'>" . $val . "</td>";
                        } elseif (in_array($field_name, ['lpp_pcs', 'lpp_rupiah', 'acost', 'frc']) || strpos($field_name, 'sls_rph_') === 0) {
    ...
    if ($is_header_printed) {
        ...
        echo "<td colspan='15' style='text-align:right;'>GRAND TOTAL:</td>";
...
<head>
    ...
    <title>Monitoring TIDAK ADA SALES - Multi IP Database</title>
    <style>
...
<div class="header-container">
    <div class="top-row">
        <div>
            <h2>Monitoring & Export Multi IP Database (TIDAK ADA SALES)</h2>
...
<div class="table-responsive" style="margin-top: 15px;">
    <table>
        ...
        <?php elseif (!empty($display_rows)): ?>
                ...
                <?php foreach ($display_rows as $r): ?>
                    <tr>
                        <?php foreach ($r as $key => $value): 
                            ...
                            $val = htmlspecialchars($value ?? '');
                        ?>
                            <?php if (in_array($field_name, ['lpp_pcs', 'lpp_rupiah', 'acost', 'frc']) || strpos($field_name, 'sls_rph_') === 0): ?>
            ...
            <tfoot>
                <tr>
                    <td colspan="15" class="text-right">GRAND TOTAL (ALL PAGES):</td>
