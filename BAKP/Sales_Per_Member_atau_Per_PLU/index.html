...
// Input Text PLU & Kode Member
$sel_plu    = isset($_GET['plu']) ? trim($_GET['plu']) : '';
$sel_member = isset($_GET['member']) ? trim($_GET['member']) : '';
...
// Helper Filter Tanggal Dinamis untuk Sales (trjd_transactiondate)
function build_sales_date_where($col_name, $sel_tgl, $sel_bulan, $sel_tahun) {
    $conds = [];
...
$where_sales_tgl = build_sales_date_where('trjd_transactiondate', $sel_tgl, $sel_bulan, $sel_tahun);

// Filter Dynamic PLU (TRJD_PRDCD) - Memakai addslashes agar tidak deprecated
$plu_array = array_filter(array_map('trim', explode(',', $sel_plu)));
if (!empty($plu_array)) {
    $escaped_plu = array_map(function($p) { return "'" . addslashes($p) . "'"; }, $plu_array);
    $where_plu = "TRJD_PRDCD IN (" . implode(',', $escaped_plu) . ")";
} else {
    $where_plu = "1=1";
}

// Filter Dynamic Member (TRJD_CUS_KODEMEMBER) - Memakai addslashes agar tidak deprecated
$member_array = array_filter(array_map('trim', explode(',', $sel_member)));
if (!empty($member_array)) {
    $escaped_member = array_map(function($m) { return "'" . addslashes($m) . "'"; }, $member_array);
    $where_member = "TRJD_CUS_KODEMEMBER IN (" . implode(',', $escaped_member) . ")";
} else {
    $where_member = "1=1";
}

// ---------------------------------------------------------
// 3. DAFTAR PILIHAN QUERY (SALES PERMEMBER / PERPLU)
// ---------------------------------------------------------
$queries = [
    'sales_member' => [
        'title' => 'Sales Permember / PerPLU',
        'qty_col' => 'qty',
        'rp_col'  => 'gros',
        'sql_detail' => "SELECT
            TO_CHAR(trjd_transactiondate,'YYYY-MM-DD HH24:MI:SS') AS TGL,
            TO_CHAR(TRJD_TRANSACTIONDATE,'YYYYMMDD')||TRJD_CASHIERSTATION||TRJD_CREATE_BY||TRJD_TRANSACTIONNO||TRJD_TRANSACTIONTYPE AS STRUK,
            TRJD_CUS_KODEMEMBER AS KD_MEM,
            CUS_NAMAMEMBER AS NAMA,
            tbtr_jualdetail.trjd_cashierstation AS KASSA,
            tbtr_jualdetail.trjd_create_by AS ID_KSR,
            tbtr_jualdetail.trjd_transactionno AS NO,
            TRJD_PRDCD AS PLU,
            ...
            (CASE                
                ...
                ELSE 0 END) AS QTY,
            (CASE
                WHEN (trjd_transactiontype='S') AND (trjd_flagtax2='Y') THEN trjd_nominalamt*1.11
        ...
        WHERE
            {$where_sales_tgl} AND
            TRJD_RECORDID IS NULL AND
            {$where_plu} AND
            {$where_member}
        ORDER BY STRUK ASC, ID_KSR ASC",

        'sql_rekap' => "SELECT
            TRJD_PRDCD AS PLU,
            PRD_DESKRIPSIPANJANG AS DESKRIPSI,
            COUNT(DISTINCT TO_CHAR(TRJD_TRANSACTIONDATE,'YYYYMMDD')||TRJD_CASHIERSTATION||TRJD_CREATE_BY||TRJD_TRANSACTIONNO||TRJD_TRANSACTIONTYPE) AS TOTAL_STRUK,
        ...
        WHERE
            ...
            {$where_plu} AND
            {$where_member}
        GROUP BY TRJD_PRDCD, PRD_DESKRIPSIPANJANG
        ORDER BY TRJD_PRDCD ASC"
    ]
];

$selected_query_key = 'sales_member';
...
if ($action == 'export_excel') {
    $filename = "Laporan_SALES_PERMEMBER_" . strtoupper($sel_view) . "_" . date('Ymd_His');
            ...
            if ($result) {
                ...
                while ($row = pg_fetch_assoc($result)) {
                    ...
                    foreach ($row as $key => $value) {
                        ...
                        if (in_array($field_name, ['plu', 'kd_mem', 'struk', 'kassa', 'id_ksr', 'no'])) {
                            echo "<td style='mso-number-format:\"\\@\";'>" . $val . "</td>";
                        } elseif (in_array($field_name, ['qty', 'gros', 'total_qty', 'total_rp', 'total_record', 'total_struk', $qty_col, $rp_col])) {
...
<head>
    ...
    <title>Monitoring Sales Permember / PerPLU - Multi IP Database</title>
    <style>
...
<div class="header-container">
    <div class="top-row">
        <div>
            <h2>Monitoring & Export Multi IP Database (Sales Permember / PerPLU)</h2>
    ...
    <form method="GET" action="" class="filter-form">
        ...
        <div class="form-group">
            ...
            <select name="view_mode" id="view_mode">
                ...
                <option value="rekap" <?= $sel_view == 'rekap' ? 'selected' : '' ?>>📊 Rekap per PLU</option>
            </select>
        </div>
        ...
        <div class="form-group">
            <label for="plu">Filter PLU (TRJD_PRDCD):</label>
        ...
        <div class="form-group">
            <label for="member">Filter Kode Member:</label>
            <input type="text" name="member" id="member" value="<?= htmlspecialchars($sel_member) ?>" placeholder="Contoh: M0001, M0002">
            <span class="help-text">Pisahkan dengan koma jika banyak</span>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="bulan[]" id="bulan" multiple>
                ...
                <?php foreach ($nama_bulan as $num => $nama): 
                    if ($num === 'ALL') continue; ?>
                    <option value="<?= (int)$num ?>" <?= in_array((string)$num, $sel_bulan) || in_array((string)(int)$num, $sel_bulan) || in_array($num, $sel_bulan) ? 'selected' : '' ?>><?= $nama ?></option>
                <?php endforeach; ?>
            </select>
            <span class="help-text">Tahan <b>Ctrl</b>/<b>Cmd</b></span>
        </div>
                ...
                <?php foreach ($display_rows as $r): ?>
                    <tr>
                        <?php foreach ($r as $key => $value): 
                            ...
                            $val = htmlspecialchars($value ?? '');
                        ?>
                            <?php if (in_array($field_name, ['qty', 'gros', 'total_qty', 'total_rp'])): ?>
                                <td class="text-right"><?= number_format((float)$value, 2, ',', '.') ?></td>
                            <?php elseif (in_array($field_name, ['total_record', 'total_struk'])): ?>
            ...
            <tfoot>
                <tr>
                    <?php if ($sel_view === 'rekap'): ?>
                        <td colspan="5" class="text-right">GRAND TOTAL (ALL PAGES):</td>
                        <td class="text-right"><?= number_format($grand_qty, 2, ',', '.') ?></td>
                        <td class="text-right"><?= number_format($grand_rp, 2, ',', '.') ?></td>
                    <?php else: ?>
                        <td colspan="10" class="text-right">GRAND TOTAL (ALL PAGES):</td>
                        <td class="text-right"><?= number_format($grand_qty, 2, ',', '.') ?></td>
                        <td class="text-right"><?= number_format($grand_rp, 2, ',', '.') ?></td>
                    <?php endif; ?>
                </tr>
