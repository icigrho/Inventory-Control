...
// Input Teks Search (PLU, NO PO, NO BTB)
$sel_plu    = isset($_GET['plu']) ? trim($_GET['plu']) : '';
$sel_nopo   = isset($_GET['nopo']) ? trim($_GET['nopo']) : '';
$sel_nobtb  = isset($_GET['nobtb']) ? trim($_GET['nobtb']) : '';
...
// Helper Filter Tanggal Dinamis untuk BTB (mstd_tgldoc) & PO (tpod_tglpo)
function build_date_where($col_name, $sel_tgl, $sel_bulan, $sel_tahun) {
    $conds = [];
    if (!in_array('ALL', $sel_tgl) && !empty($sel_tgl)) {
        $clean_tgl = array_map('intval', $sel_tgl);
        $conds[] = "EXTRACT(DAY FROM {$col_name}) IN (" . implode(',', $clean_tgl) . ")";
    }
    if (!in_array('ALL', $sel_bulan) && !empty($sel_bulan)) {
        $clean_bulan = array_map('intval', $sel_bulan);
        $conds[] = "EXTRACT(MONTH FROM {$col_name}) IN (" . implode(',', $clean_bulan) . ")";
    }
    if ($sel_tahun !== 'ALL') {
        $conds[] = "EXTRACT(YEAR FROM {$col_name}) = " . (int)$sel_tahun;
    }
    return count($conds) > 0 ? implode(' AND ', $conds) : "1=1";
}

// Helper Filter Teks Dinamis untuk Klausa IN (Mendukung Input Terpisah Koma)
function build_text_where($col_name, $input_str) {
    if (empty($input_str)) return "1=1";
    $items = array_filter(array_map('trim', explode(',', $input_str)));
    if (empty($items)) return "1=1";
    $escaped = array_map(function($val) {
        return "'" . addslashes($val) . "'";
    }, $items);
    return "{$col_name} IN (" . implode(',', $escaped) . ")";
}

// Tanggal Condition
$where_btb_tgl = build_date_where('mstd_tgldoc', $sel_tgl, $sel_bulan, $sel_tahun);
$where_po_tgl  = build_date_where('tpod_tglpo', $sel_tgl, $sel_bulan, $sel_tahun);

// Teks Input Condition untuk BTB
$where_btb_plu   = build_text_where('mstd_prdcd', $sel_plu);
$where_btb_nopo  = build_text_where('mstd_nopo', $sel_nopo);
$where_btb_nobtb = build_text_where('mstd_nodoc', $sel_nobtb);
$where_btb_text  = "{$where_btb_plu} AND {$where_btb_nopo} AND {$where_btb_nobtb}";

// Teks Input Condition untuk PO
$where_po_plu    = build_text_where('tpod_prdcd', $sel_plu);
$where_po_nopo   = build_text_where('tpod_nopo', $sel_nopo);
$where_po_text   = "{$where_po_plu} AND {$where_po_nopo}";

// ---------------------------------------------------------
// 3. DAFTAR PILIHAN QUERY (BTB DAN PO)
// ---------------------------------------------------------
$queries = [
    'btb' => [
        'title' => 'BTB',
        ...
        'sql_detail' => "SELECT 
            CASE 
                WHEN prd_kodeigr='01' THEN 'IGR CPG' 
                WHEN prd_kodeigr='03' THEN 'IGR SBY'
                WHEN prd_kodeigr='04' THEN 'IGR BDG'
                WHEN prd_kodeigr='05' THEN 'IGR TGR'
                WHEN prd_kodeigr='06' THEN 'IGR YOG'
                WHEN prd_kodeigr='15' THEN 'IGR MDN'
                WHEN prd_kodeigr='16' THEN 'IGR BKS'
                WHEN prd_kodeigr='17' THEN 'IGR PLG'
                WHEN prd_kodeigr='18' THEN 'IGR KMY'
                WHEN prd_kodeigr='20' THEN 'IGR PKU'
                WHEN prd_kodeigr='21' THEN 'IGR SMD'
                WHEN prd_kodeigr='22' THEN 'IGR SMG'
                WHEN prd_kodeigr='25' THEN 'IGR BGR'
                WHEN prd_kodeigr='26' THEN 'IGR PTK'
                WHEN prd_kodeigr='27' THEN 'IGR BMS'
                WHEN prd_kodeigr='28' THEN 'IGR MDO'
                WHEN prd_kodeigr='31' THEN 'IGR MKS'
                WHEN prd_kodeigr='32' THEN 'IGR JBI'
                WHEN prd_kodeigr='33' THEN 'IGR KRI'
                WHEN prd_kodeigr='34' THEN 'IGR AMB'
                WHEN prd_kodeigr='35' THEN 'IGR CPT'
                WHEN prd_kodeigr='36' THEN 'IGR KRW'
                WHEN prd_kodeigr='37' THEN 'IGR MLG'
                WHEN prd_kodeigr='38' THEN 'IGR BDL'
                WHEN prd_kodeigr='39' THEN 'IGR SLO'
                WHEN prd_kodeigr='43' THEN 'IGR SBI'
                WHEN prd_kodeigr='44' THEN 'IGR CKL'
                WHEN prd_kodeigr='46' THEN 'IGR BTM'
                WHEN prd_kodeigr='47' THEN 'IGR PWT'
                WHEN prd_kodeigr='50' THEN 'IGR GTO'
                WHEN prd_kodeigr='48' THEN 'IGR BLI'
                WHEN prd_kodeigr='51' THEN 'IGR PDG'
                WHEN prd_kodeigr='7A' THEN 'ICM CKL'
                WHEN prd_kodeigr='7B' THEN 'ICM SBI'
                WHEN prd_kodeigr='7D' THEN 'ICM TGR'
                WHEN prd_kodeigr='7G' THEN 'ICM YOG'
                WHEN prd_kodeigr='7K' THEN 'ICM PLG'
                WHEN prd_kodeigr='7H' THEN 'ICM BGR'
                WHEN prd_kodeigr='7F' THEN 'ICM MLG'
                WHEN prd_kodeigr='7P' THEN 'ICM AMB'
                WHEN prd_kodeigr='7L' THEN 'ICM KRW'
                WHEN prd_kodeigr='7E' THEN 'ICM BDL'
                WHEN prd_kodeigr='7S' THEN 'ICM PWT'
                WHEN prd_kodeigr='7R' THEN 'ICM SMG'
                WHEN prd_kodeigr='7N' THEN 'ICM KRI'
                WHEN prd_kodeigr='7M' THEN 'ICM PKU'
                WHEN prd_kodeigr='7C' THEN 'ICM BTM'
                WHEN prd_kodeigr='7I' THEN 'ICM SMD'
                WHEN prd_kodeigr='7O' THEN 'ICM KMY'
                WHEN prd_kodeigr='7J' THEN 'ICM BMS'
                WHEN prd_kodeigr='7U' THEN 'ICM PTK'
                WHEN prd_kodeigr='7V' THEN 'ICM SLO'
                WHEN prd_kodeigr='7X' THEN 'ICM GTO'
                WHEN prd_kodeigr='7Q' THEN 'ICM BDG'
                WHEN prd_kodeigr='7W' THEN 'ICM MDO'
                WHEN prd_kodeigr='8B' THEN 'ICM BLI'
                WHEN prd_kodeigr='7Z' THEN 'ICM SBY'
                WHEN prd_kodeigr='7Y' THEN 'ICM CPG'
                WHEN prd_kodeigr='1A' THEN 'SPI BLJ'
                WHEN prd_kodeigr='1B' THEN 'SPI MKS'
                WHEN prd_kodeigr='1C' THEN 'SPI KRW'
                WHEN prd_kodeigr='1D' THEN 'SPI SBY'
                WHEN prd_kodeigr='1E' THEN 'SPI MDN'
                WHEN prd_kodeigr='1F' THEN 'SPI MDO'
                WHEN prd_kodeigr='1G' THEN 'SPI BDG'
                WHEN prd_kodeigr='1H' THEN 'SPI MLG'
                WHEN prd_kodeigr='1I' THEN 'SPI CPG'
                WHEN prd_kodeigr='1K' THEN 'SPI BKS'
                WHEN prd_kodeigr='1L' THEN 'SPI SMD'
                WHEN prd_kodeigr='1M' THEN 'SPI BMS'
                WHEN prd_kodeigr='1N' THEN 'SPI SMG'
                WHEN prd_kodeigr='1Q' THEN 'SPI BGR'
                WHEN prd_kodeigr='1J' THEN 'SPI SLO'
                WHEN prd_kodeigr='1P' THEN 'SPI TGR2'
                WHEN prd_kodeigr='1S' THEN 'SPI CKL'
                WHEN prd_kodeigr='1R' THEN 'SPI BDL'
                WHEN prd_kodeigr='1T' THEN 'SPI SBI'
                WHEN prd_kodeigr='1O' THEN 'SPI YOG'
                WHEN prd_kodeigr='1U' THEN 'SPI BDG2'
                WHEN prd_kodeigr='1V' THEN 'SPI BDG3'
                WHEN prd_kodeigr='1W' THEN 'SPI BDG4'
                WHEN prd_kodeigr='1Y' THEN 'SPI BDG5'
                WHEN prd_kodeigr='2B' THEN 'SPI CKL2'
                WHEN prd_kodeigr='2G' THEN 'SPI SLO2'
                WHEN prd_kodeigr='2H' THEN 'SPI BDG6'
                WHEN prd_kodeigr='2A' THEN 'SPI CPT'
                WHEN prd_kodeigr='2D' THEN 'SPI SBY2'
                WHEN prd_kodeigr='2E' THEN 'SPI SBY3'
                WHEN prd_kodeigr='2F' THEN 'SPI SMG3'
                WHEN prd_kodeigr='1Z' THEN 'SPI KMY'
                WHEN prd_kodeigr='2L' THEN 'SPI CKL3'
                WHEN prd_kodeigr='2N' THEN 'SPI SLO3'
                WHEN prd_kodeigr='2I' THEN 'SPI SMG4'
                WHEN prd_kodeigr='2J' THEN 'SPI SMG5'
                WHEN prd_kodeigr='2O' THEN 'SPI SBY4'
                WHEN prd_kodeigr='2K' THEN 'SPI BDG7'
                WHEN prd_kodeigr='2C' THEN 'SPI MLG2'
                WHEN prd_kodeigr='AA' THEN 'SPIX BKS'
                WHEN prd_kodeigr='AB' THEN 'SPIX BGR'
                WHEN prd_kodeigr='AC' THEN 'SPIX SMG'
                WHEN prd_kodeigr='2P' THEN 'SPI BDG8'
                WHEN prd_kodeigr='2Q' THEN 'SPI MLG4'
                WHEN prd_kodeigr='2R' THEN 'SPI CKL4'
                WHEN prd_kodeigr='2M' THEN 'SPI MLG3'
                WHEN prd_kodeigr='2V' THEN 'SPI PKU'
                WHEN prd_kodeigr='2X' THEN 'SPI SBY5'
                WHEN prd_kodeigr='2U' THEN 'SPI BDL2'
                WHEN prd_kodeigr='2T' THEN 'SPI BDG9'
                WHEN prd_kodeigr='2Y' THEN 'SPI SMG6'
                WHEN prd_kodeigr='2Z' THEN 'SPI YOG2'
                WHEN prd_kodeigr='2W' THEN 'SPI SBI2'
                WHEN prd_kodeigr='2S' THEN 'SPI MDN2'
                WHEN prd_kodeigr='3A' THEN 'SPI TGR3'
                WHEN prd_kodeigr='3B' THEN 'SPI BGR2'
                WHEN prd_kodeigr='3C' THEN 'SPI MLG5'
                WHEN prd_kodeigr='3D' THEN 'SPI PWT'
                WHEN prd_kodeigr='3E' THEN 'SPI BDG10'
                WHEN prd_kodeigr='3F' THEN 'SPI SLO4'
                WHEN prd_kodeigr='3O' THEN 'SPI BKS3'
                WHEN prd_kodeigr='3L' THEN 'SPI BKS2'
                WHEN prd_kodeigr='4C' THEN 'SPI BMS2'
                WHEN prd_kodeigr='3U' THEN 'SPI BGR4'
                WHEN prd_kodeigr='3M' THEN 'SPI CKL5'
                WHEN prd_kodeigr='3K' THEN 'SPI SMG7'
                WHEN prd_kodeigr='3J' THEN 'SPI SMD2'
                WHEN prd_kodeigr='3I' THEN 'SPI SBI3'
                WHEN prd_kodeigr='3N' THEN 'SPI SBY6'
                WHEN prd_kodeigr='4B' THEN 'SPI MLG7'
                WHEN prd_kodeigr='3X' THEN 'SPI BKS4'
                WHEN prd_kodeigr='4G' THEN 'SPI BLI2'
                WHEN prd_kodeigr='4D' THEN 'SPI KRI2'
                WHEN prd_kodeigr='4E' THEN 'SPI BMS3'
                WHEN prd_kodeigr='4L' THEN 'SPI CPG2'
                WHEN prd_kodeigr='3R' THEN 'SPI JBI'
                WHEN prd_kodeigr='4A' THEN 'SPI SBY8'
                WHEN prd_kodeigr='3G' THEN 'SPI BGR3'
                WHEN prd_kodeigr='3Y' THEN 'SPI MLG6'
                WHEN prd_kodeigr='4I' THEN 'SPI MLG8'
                WHEN prd_kodeigr='4M' THEN 'SPI PTK'
                WHEN prd_kodeigr='4H' THEN 'SPI SBY9'
                WHEN prd_kodeigr='3S' THEN 'SPI TGR4'
                WHEN prd_kodeigr='3T' THEN 'SPI AMB'
                WHEN prd_kodeigr='7T' THEN 'ICM CPT'
                WHEN prd_kodeigr='3Q' THEN 'SPI KRI'
                WHEN prd_kodeigr='4J' THEN 'SPI KRW2'
                WHEN prd_kodeigr='4N' THEN 'SPI KRW3'
                WHEN prd_kodeigr='4K' THEN 'SPI MDO'
                WHEN prd_kodeigr='3H' THEN 'SPI PLG'
                WHEN prd_kodeigr='3W' THEN 'SPI PLG2'
                WHEN prd_kodeigr='4P' THEN 'SPI KMY2'
                WHEN prd_kodeigr='4X' THEN 'SPI BMS4'
                WHEN prd_kodeigr='3V' THEN 'SPI MKS2'
                WHEN prd_kodeigr='4Q' THEN 'SPI SMD3'
                WHEN prd_kodeigr='4F' THEN 'SPI BLI'
            END AS CAB,
            mstd_typetrn AS TYPE,
            mstd_nopo AS NO_PO,
            TO_CHAR(mstd_tglpo, 'yyyy-mm-dd') AS TGL_PO,
            mstd_nodoc AS NO_BTB,
            TO_CHAR(mstd_tgldoc, 'yyyy-mm-dd') AS TGL_BTB,
            mstd_prdcd AS PLU,
            ...
            mstd_qty AS QTY_BTB,
            mstd_gross AS RP_BTB
        FROM tbtr_mstran_d 
        LEFT JOIN tbmaster_prodmast ON prd_prdcd = mstd_prdcd
        WHERE {$where_btb_tgl} AND {$where_btb_text} AND mstd_typetrn = 'B' 
        ORDER BY mstd_tglpo DESC, mstd_nodoc",

        'sql_rekap' => "SELECT 
            prd_kodeigr AS KODE_CABANG,
            COUNT(DISTINCT mstd_nodoc) AS TOTAL_BTB,
            COUNT(1) AS TOTAL_RECORD,
            SUM(mstd_qty) AS QTY_BTB,
            SUM(mstd_gross) AS RP_BTB
        FROM tbtr_mstran_d 
        LEFT JOIN tbmaster_prodmast ON prd_prdcd = mstd_prdcd
        WHERE {$where_btb_tgl} AND {$where_btb_text} AND mstd_typetrn = 'B' 
        GROUP BY prd_kodeigr
        ORDER BY prd_kodeigr"
    ],
    ...
    'po' => [
        ...
        'sql_detail' => "SELECT 
            CASE 
                WHEN tpod_kodeigr='01' THEN 'IGR CPG' 
                WHEN tpod_kodeigr='03' THEN 'IGR SBY'
                WHEN tpod_kodeigr='04' THEN 'IGR BDG'
                WHEN tpod_kodeigr='05' THEN 'IGR TGR'
                WHEN tpod_kodeigr='06' THEN 'IGR YOG'
                WHEN tpod_kodeigr='15' THEN 'IGR MDN'
                WHEN tpod_kodeigr='16' THEN 'IGR BKS'
                WHEN tpod_kodeigr='17' THEN 'IGR PLG'
                WHEN tpod_kodeigr='18' THEN 'IGR KMY'
                WHEN tpod_kodeigr='20' THEN 'IGR PKU'
                WHEN tpod_kodeigr='21' THEN 'IGR SMD'
                WHEN tpod_kodeigr='22' THEN 'IGR SMG'
                WHEN tpod_kodeigr='25' THEN 'IGR BGR'
                WHEN tpod_kodeigr='26' THEN 'IGR PTK'
                WHEN tpod_kodeigr='27' THEN 'IGR BMS'
                WHEN tpod_kodeigr='28' THEN 'IGR MDO'
                WHEN tpod_kodeigr='31' THEN 'IGR MKS'
                WHEN tpod_kodeigr='32' THEN 'IGR JBI'
                WHEN tpod_kodeigr='33' THEN 'IGR KRI'
                WHEN tpod_kodeigr='34' THEN 'IGR AMB'
                WHEN tpod_kodeigr='35' THEN 'IGR CPT'
                WHEN tpod_kodeigr='36' THEN 'IGR KRW'
                WHEN tpod_kodeigr='37' THEN 'IGR MLG'
                WHEN tpod_kodeigr='38' THEN 'IGR BDL'
                WHEN tpod_kodeigr='39' THEN 'IGR SLO'
                WHEN tpod_kodeigr='43' THEN 'IGR SBI'
                WHEN tpod_kodeigr='44' THEN 'IGR CKL'
                WHEN tpod_kodeigr='46' THEN 'IGR BTM'
                WHEN tpod_kodeigr='47' THEN 'IGR PWT'
                WHEN tpod_kodeigr='50' THEN 'IGR GTO'
                WHEN tpod_kodeigr='48' THEN 'IGR BLI'
                WHEN tpod_kodeigr='51' THEN 'IGR PDG'
                WHEN tpod_kodeigr='7A' THEN 'ICM CKL'
                WHEN tpod_kodeigr='7B' THEN 'ICM SBI'
                WHEN tpod_kodeigr='7D' THEN 'ICM TGR'
                WHEN tpod_kodeigr='7G' THEN 'ICM YOG'
                WHEN tpod_kodeigr='7K' THEN 'ICM PLG'
                WHEN tpod_kodeigr='7H' THEN 'ICM BGR'
                WHEN tpod_kodeigr='7F' THEN 'ICM MLG'
                WHEN tpod_kodeigr='7P' THEN 'ICM AMB'
                WHEN tpod_kodeigr='7L' THEN 'ICM KRW'
                WHEN tpod_kodeigr='7E' THEN 'ICM BDL'
                WHEN tpod_kodeigr='7S' THEN 'ICM PWT'
                WHEN tpod_kodeigr='7R' THEN 'ICM SMG'
                WHEN tpod_kodeigr='7N' THEN 'ICM KRI'
                WHEN tpod_kodeigr='7M' THEN 'ICM PKU'
                WHEN tpod_kodeigr='7C' THEN 'ICM BTM'
                WHEN tpod_kodeigr='7I' THEN 'ICM SMD'
                WHEN tpod_kodeigr='7O' THEN 'ICM KMY'
                WHEN tpod_kodeigr='7J' THEN 'ICM BMS'
                WHEN tpod_kodeigr='7U' THEN 'ICM PTK'
                WHEN tpod_kodeigr='7V' THEN 'ICM SLO'
                WHEN tpod_kodeigr='7X' THEN 'ICM GTO'
                WHEN tpod_kodeigr='7Q' THEN 'ICM BDG'
                WHEN tpod_kodeigr='7W' THEN 'ICM MDO'
                WHEN tpod_kodeigr='8B' THEN 'ICM BLI'
                WHEN tpod_kodeigr='7Z' THEN 'ICM SBY'
                WHEN tpod_kodeigr='7Y' THEN 'ICM CPG'
                WHEN tpod_kodeigr='1A' THEN 'SPI BLJ'
                WHEN tpod_kodeigr='1B' THEN 'SPI MKS'
                WHEN tpod_kodeigr='1C' THEN 'SPI KRW'
                WHEN tpod_kodeigr='1D' THEN 'SPI SBY'
                WHEN tpod_kodeigr='1E' THEN 'SPI MDN'
                WHEN tpod_kodeigr='1F' THEN 'SPI MDO'
                WHEN tpod_kodeigr='1G' THEN 'SPI BDG'
                WHEN tpod_kodeigr='1H' THEN 'SPI MLG'
                WHEN tpod_kodeigr='1I' THEN 'SPI CPG'
                WHEN tpod_kodeigr='1K' THEN 'SPI BKS'
                WHEN tpod_kodeigr='1L' THEN 'SPI SMD'
                WHEN tpod_kodeigr='1M' THEN 'SPI BMS'
                WHEN tpod_kodeigr='1N' THEN 'SPI SMG'
                WHEN tpod_kodeigr='1Q' THEN 'SPI BGR'
                WHEN tpod_kodeigr='1J' THEN 'SPI SLO'
                WHEN tpod_kodeigr='1P' THEN 'SPI TGR2'
                WHEN tpod_kodeigr='1S' THEN 'SPI CKL'
                WHEN tpod_kodeigr='1R' THEN 'SPI BDL'
                WHEN tpod_kodeigr='1T' THEN 'SPI SBI'
                WHEN tpod_kodeigr='1O' THEN 'SPI YOG'
                WHEN tpod_kodeigr='1U' THEN 'SPI BDG2'
                WHEN tpod_kodeigr='1V' THEN 'SPI BDG3'
                WHEN tpod_kodeigr='1W' THEN 'SPI BDG4'
                WHEN tpod_kodeigr='1Y' THEN 'SPI BDG5'
                WHEN tpod_kodeigr='2B' THEN 'SPI CKL2'
                WHEN tpod_kodeigr='2G' THEN 'SPI SLO2'
                WHEN tpod_kodeigr='2H' THEN 'SPI BDG6'
                WHEN tpod_kodeigr='2A' THEN 'SPI CPT'
                WHEN tpod_kodeigr='2D' THEN 'SPI SBY2'
                WHEN tpod_kodeigr='2E' THEN 'SPI SBY3'
                WHEN tpod_kodeigr='2F' THEN 'SPI SMG3'
                WHEN tpod_kodeigr='1Z' THEN 'SPI KMY'
                WHEN tpod_kodeigr='2L' THEN 'SPI CKL3'
                WHEN tpod_kodeigr='2N' THEN 'SPI SLO3'
                WHEN tpod_kodeigr='2I' THEN 'SPI SMG4'
                WHEN tpod_kodeigr='2J' THEN 'SPI SMG5'
                WHEN tpod_kodeigr='2O' THEN 'SPI SBY4'
                WHEN tpod_kodeigr='2K' THEN 'SPI BDG7'
                WHEN tpod_kodeigr='2C' THEN 'SPI MLG2'
                WHEN tpod_kodeigr='AA' THEN 'SPIX BKS'
                WHEN tpod_kodeigr='AB' THEN 'SPIX BGR'
                WHEN tpod_kodeigr='AC' THEN 'SPIX SMG'
                WHEN tpod_kodeigr='2P' THEN 'SPI BDG8'
                WHEN tpod_kodeigr='2Q' THEN 'SPI MLG4'
                WHEN tpod_kodeigr='2R' THEN 'SPI CKL4'
                WHEN tpod_kodeigr='2M' THEN 'SPI MLG3'
                WHEN tpod_kodeigr='2V' THEN 'SPI PKU'
                WHEN tpod_kodeigr='2X' THEN 'SPI SBY5'
                WHEN tpod_kodeigr='2U' THEN 'SPI BDL2'
                WHEN tpod_kodeigr='2T' THEN 'SPI BDG9'
                WHEN tpod_kodeigr='2Y' THEN 'SPI SMG6'
                WHEN tpod_kodeigr='2Z' THEN 'SPI YOG2'
                WHEN tpod_kodeigr='2W' THEN 'SPI SBI2'
                WHEN tpod_kodeigr='2S' THEN 'SPI MDN2'
                WHEN tpod_kodeigr='3A' THEN 'SPI TGR3'
                WHEN tpod_kodeigr='3B' THEN 'SPI BGR2'
                WHEN tpod_kodeigr='3C' THEN 'SPI MLG5'
                WHEN tpod_kodeigr='3D' THEN 'SPI PWT'
                WHEN tpod_kodeigr='3E' THEN 'SPI BDG10'
                WHEN tpod_kodeigr='3F' THEN 'SPI SLO4'
                WHEN tpod_kodeigr='3O' THEN 'SPI BKS3'
                WHEN tpod_kodeigr='3L' THEN 'SPI BKS2'
                WHEN tpod_kodeigr='4C' THEN 'SPI BMS2'
                WHEN tpod_kodeigr='3U' THEN 'SPI BGR4'
                WHEN tpod_kodeigr='3M' THEN 'SPI CKL5'
                WHEN tpod_kodeigr='3K' THEN 'SPI SMG7'
                WHEN tpod_kodeigr='3J' THEN 'SPI SMD2'
                WHEN tpod_kodeigr='3I' THEN 'SPI SBI3'
                WHEN tpod_kodeigr='3N' THEN 'SPI SBY6'
                WHEN tpod_kodeigr='4B' THEN 'SPI MLG7'
                WHEN tpod_kodeigr='3X' THEN 'SPI BKS4'
                WHEN tpod_kodeigr='4G' THEN 'SPI BLI2'
                WHEN tpod_kodeigr='4D' THEN 'SPI KRI2'
                WHEN tpod_kodeigr='4E' THEN 'SPI BMS3'
                WHEN tpod_kodeigr='4L' THEN 'SPI CPG2'
                WHEN tpod_kodeigr='3R' THEN 'SPI JBI'
                WHEN tpod_kodeigr='4A' THEN 'SPI SBY8'
                WHEN tpod_kodeigr='3G' THEN 'SPI BGR3'
                WHEN tpod_kodeigr='3Y' THEN 'SPI MLG6'
                WHEN tpod_kodeigr='4I' THEN 'SPI MLG8'
                WHEN tpod_kodeigr='4M' THEN 'SPI PTK'
                WHEN tpod_kodeigr='4H' THEN 'SPI SBY9'
                WHEN tpod_kodeigr='3S' THEN 'SPI TGR4'
                WHEN tpod_kodeigr='3T' THEN 'SPI AMB'
                WHEN tpod_kodeigr='7T' THEN 'ICM CPT'
                WHEN tpod_kodeigr='3Q' THEN 'SPI KRI'
                WHEN tpod_kodeigr='4J' THEN 'SPI KRW2'
                WHEN tpod_kodeigr='4N' THEN 'SPI KRW3'
                WHEN tpod_kodeigr='4K' THEN 'SPI MDO'
                WHEN tpod_kodeigr='3H' THEN 'SPI PLG'
                WHEN tpod_kodeigr='3W' THEN 'SPI PLG2'
                WHEN tpod_kodeigr='3P' THEN 'SPI CPT2'
                WHEN tpod_kodeigr='4P' THEN 'SPI KMY2'
                WHEN tpod_kodeigr='4X' THEN 'SPI BMS4'
                WHEN tpod_kodeigr='3V' THEN 'SPI MKS2'
                WHEN tpod_kodeigr='4Q' THEN 'SPI SMD3'
                WHEN tpod_kodeigr='4F' THEN 'SPI BLI'
            END AS CAB,
            TPOD_NOPO AS NO_PO,
            TPOD_RECORDID AS RECID,
            TO_CHAR(TPOD_TGLPO, 'yyyy-mm-dd') AS TGL_PO,
            TPOD_PRDCD AS PLU,
            PRD_KODEDIVISI AS DIV, 
            PRD_KODEDEPARTEMENT AS DEP, 
            PRD_KODEKATEGORIBARANG AS KAT,
        ...
        WHERE {$where_po_tgl} AND {$where_po_text}
        ORDER BY TPOD_TGLPO DESC, TPOD_NOPO",

        'sql_rekap' => "SELECT 
            tpod_kodeigr AS KODE_CABANG,
            COUNT(DISTINCT TPOD_NOPO) AS TOTAL_PO,
            COUNT(1) AS TOTAL_RECORD,
            SUM(TPOD_QTYPO) AS QTY_PO,
            SUM(TPOD_GROSS) AS RP_PO
        FROM TBTR_po_d 
        LEFT JOIN TBMASTER_PRODMAST ON PRD_PRDCD = TPOD_PRDCD
        WHERE {$where_po_tgl} AND {$where_po_text}
        GROUP BY tpod_kodeigr
        ORDER BY tpod_kodeigr"
    ]
];

$selected_query_key = isset($_GET['query_type']) && isset($queries[$_GET['query_type']]) ? $_GET['query_type'] : 'btb';
...
if ($action == 'export_excel') {
            ...
            if ($result) {
                ...
                while ($row = pg_fetch_assoc($result)) {
                    ...
                    foreach ($row as $key => $value) {
                        ...
                        if (in_array($field_name, ['plu', 'div', 'dep', 'kat', 'kode_cabang', 'no_po', 'no_btb', 'recid'])) {
                            echo "<td style='mso-number-format:\"\\@\";'>" . $val . "</td>";
                        } elseif (in_array($field_name, ['qty_btb', 'rp_btb', 'acost', 'qty_po', 'rp_po', 'total_qty', 'total_rp', 'total_record', 'total_btb', 'total_po', $qty_col, $rp_col])) {
    ...
    if ($is_header_printed) {
        ...
        if ($sel_view === 'rekap') {
            ...
            $colspan = ($selected_query_key === 'po') ? 9 : 11;
            echo "<td colspan='{$colspan}' style='text-align:right;'>GRAND TOTAL:</td>";
            echo "<td style='text-align:right;'>" . number_format($grand_qty, 2, '.', '') . "</td>";
            echo "<td style='text-align:right;'>" . number_format($grand_rp, 2, '.', '') . "</td>";
            if ($selected_query_key === 'btb') echo "<td colspan='1'></td>";
        }
        echo "</tr>";
    }
...
<head>
    ...
    <title>Monitoring Multi IP Database - <?= strtoupper($selected_query_key) ?></title>
    <style>
...
<div class="header-container">
    ...
    <form method="GET" action="" class="filter-form">
        ...
        <!-- SEARCH INPUT FIELDS: PLU, NO PO, NO BTB -->
        <div class="form-group">
            <label for="plu">Filter PLU:</label>
            <input type="text" name="plu" id="plu" value="<?= htmlspecialchars($sel_plu) ?>" placeholder="Misal: 0431090, 0431091">
            <span class="help-text">Pisahkan koma jika banyak</span>
        </div>

        <div class="form-group">
            <label for="nopo">Filter NO PO:</label>
            <input type="text" name="nopo" id="nopo" value="<?= htmlspecialchars($sel_nopo) ?>" placeholder="Misal: PO123, PO124">
            <span class="help-text">Pisahkan koma jika banyak</span>
        </div>

        <div class="form-group">
            <label for="nobtb">Filter NO BTB:</label>
            <input type="text" name="nobtb" id="nobtb" value="<?= htmlspecialchars($sel_nobtb) ?>" placeholder="Misal: BTB001, BTB002">
            <span class="help-text">Pisahkan koma jika banyak</span>
        </div>
                ...
                <?php foreach ($display_rows as $r): ?>
                    <tr>
                        <?php foreach ($r as $key => $value): 
                            ...
                            $val = htmlspecialchars($value ?? '');
                        ?>
                            <?php if (in_array($field_name, ['qty_btb', 'rp_btb', 'acost', 'qty_po', 'rp_po', 'total_qty', 'total_rp'])): ?>
                                <td class="text-right"><?= number_format((float)$value, 2, ',', '.') ?></td>
                            <?php elseif (in_array($field_name, ['total_record', 'total_btb', 'total_po'])): ?>
            ...
            <tfoot>
                <tr>
                    <?php if ($sel_view === 'rekap'): ?>
                        <td colspan="4" class="text-right">GRAND TOTAL (ALL PAGES):</td>
                        <td class="text-right"><?= number_format($grand_qty, 2, ',', '.') ?></td>
                        <td class="text-right"><?= number_format($grand_rp, 2, ',', '.') ?></td>
                    <?php else: ?>
                        <?php $colspan = ($selected_query_key === 'po') ? 9 : 11; ?>
                        <td colspan="<?= $colspan ?>" class="text-right">GRAND TOTAL (ALL PAGES):</td>
                        <td class="text-right"><?= number_format($grand_qty, 2, ',', '.') ?></td>
                        <td class="text-right"><?= number_format($grand_rp, 2, ',', '.') ?></td>
                        <?php if ($selected_query_key === 'btb'): ?><td colspan="1"></td><?php endif; ?>
                    <?php endif; ?>
                </tr>
            </tfoot>
        <?php else: ?>
