<?php
// =========================================================
// MONITORING LPP - REKAP MULTI SERVER IGR / SPI / ICM
// Versi revisi: hanya menjalankan query REKAP ringan.
// =========================================================
...
// ---------------------------------------------------------
// 4. FILTER PLU YANG AMAN
//    PLU dibandingkan sebagai TEXT agar leading zero tetap terjaga.
// ---------------------------------------------------------
$plu_array = [];
if ($sel_plu !== '') {
    foreach (preg_split('/[,\s;]+/', $sel_plu) as $plu) {
        $plu = trim($plu);
        if ($plu === '') continue;
        // Hanya karakter PLU yang lazim: angka / huruf.
...
$where_plu = '1=1';
if (!empty($plu_array)) {
    $quoted = array_map('sql_quote', $plu_array);
    $where_plu = 'st_prdcd::text IN (' . implode(',', $quoted) . ')';
}

// ---------------------------------------------------------
// 5. QUERY REKAP RINGAN
//    Tidak memakai JOIN lokasi/sales/supplier/KKPKM/MSTRAN.
//    1 query sederhana per server.
// ---------------------------------------------------------
function build_rekap_sql($where_plu) {
    return "SELECT
        COUNT(DISTINCT st_prdcd) AS total_item,
        COALESCE(SUM(st_saldoakhir), 0) AS total_qty,
        COALESCE(SUM(
            CASE
                WHEN prd_unit = 'KG' THEN (COALESCE(st_avgcost,0) / 1000.0) * COALESCE(st_saldoakhir,0)
                ELSE COALESCE(st_avgcost,0) * COALESCE(st_saldoakhir,0)
            END
        ), 0) AS total_rupiah
    FROM tbmaster_stock
...
// ---------------------------------------------------------
// 6. DOWNLOAD DETAIL SESUAI QUERY AWAL
//    Query detail hanya dijalankan saat tombol DOWNLOAD DETAIL
//    ditekan, sehingga tidak membebani tampilan rekap awal.
// ---------------------------------------------------------
$action = isset($_GET['action']) ? $_GET['action'] : '';

function build_detail_sql($where_plu) {
    return "SELECT
        CASE
    when prd_kodeigr='01' then 'CPG' 
    when prd_kodeigr='03' then 'SBY'
    when prd_kodeigr='04' then 'BDG'
    when prd_kodeigr='05' then 'TGR'
    when prd_kodeigr='06' then 'YOG'
    when prd_kodeigr='15' then 'MDN'
    when prd_kodeigr='16' then 'BKS'
    when prd_kodeigr='17' then 'PLG'
    when prd_kodeigr='18' then 'KMY'
    when prd_kodeigr='20' then 'PKU'
    when prd_kodeigr='21' then 'SMD'
    when prd_kodeigr='22' then 'SMG'
    when prd_kodeigr='25' then 'BGR'
    when prd_kodeigr='26' then 'PTK'
    when prd_kodeigr='27' then 'BMS'
    when prd_kodeigr='28' then 'MDO'
    when prd_kodeigr='31' then 'MKS'
    when prd_kodeigr='32' then 'JBI'
    when prd_kodeigr='33' then 'KRI'
    when prd_kodeigr='34' then 'AMB'
    when prd_kodeigr='35' then 'CPT'
    when prd_kodeigr='36' then 'KRW'
    when prd_kodeigr='37' then 'MLG'
    when prd_kodeigr='38' then 'BDL'
    when prd_kodeigr='39' then 'SLO'
    when prd_kodeigr='43' then 'SBI'
    when prd_kodeigr='44' then 'CKL'
    when prd_kodeigr='46' then 'BTM'
    when prd_kodeigr='47' then 'PWT'
    when prd_kodeigr='50' then 'GTO'
    when prd_kodeigr='48' then 'BLI'
    ...
    when prd_kodeigr='AA' then 'SPI-X BKS'
    when prd_kodeigr='AB' then 'SPI-X BGR'
    when prd_kodeigr='AC' then 'SPI-X SMG'
	...
	when prd_kodeigr='3P' then 'SPI CPT2'
	...
	when prd_kodeigr='3V' then 'SPI MKS2'
        ...
        prd_unit AS sat,
        prd_frac AS frc,
        prd_perlakuanbarang AS stt,
        ROUND(st_avgcost) AS accost,
        ...
        sls_qty_03 AS sls_qty_06,
        sls_qty_02 AS sls_qty_07,
        sls_qty_01 AS sls_qty_08,
        sls_rph_03 AS sls_rph_06,
        sls_rph_02 AS sls_rph_07,
        sls_rph_01 AS sls_rph_08,
        sls_hpp_03 AS sls_hpp_06,
        sls_hpp_02 AS sls_hpp_07,
        sls_hpp_01 AS sls_hpp_08,
        sup_top AS top,
        prd_create_dt AS tgl_aktif,
        TO_CHAR(bpbahkir.bpbterakhir,'DD/MM/YYYY') AS tglbpb,
        lks_expdate AS tgl_ed,
        prd_tgldiscontinue AS tgl_tag,
        sup_namasupplier AS namasupplier
    FROM tbmaster_stock
    LEFT JOIN (SELECT * FROM tbmaster_lokasi WHERE lks_tiperak NOT IN ('Z','S')) type
        ON st_prdcd=lks_prdcd
    LEFT JOIN tbmaster_prodmast ON st_prdcd=prd_prdcd
    LEFT JOIN tbtr_salesbulanan ON sls_prdcd=prd_prdcd
    LEFT JOIN tbmaster_kkpkm ON prd_prdcd=pkm_prdcd
    LEFT JOIN (
        SELECT mstd_prdcd AS plpb, MAX(mstd_tgldoc) AS bpbterakhir
        FROM tbtr_mstran_d
        WHERE mstd_typetrn='B'
        GROUP BY mstd_prdcd
    ) bpbahkir ON st_prdcd=bpbahkir.plpb
...
if ($action === 'export_detail' && $is_searched) {
    set_time_limit(0);
    $filename = 'Detail_LPP_' . date('Ymd_His') . '.xls';
    header('Content-Type: application/vnd.ms-excel; charset=utf-8');
    header('Content-Disposition: attachment; filename="'.$filename.'"');
    header('Pragma: no-cache');
    header('Expires: 0');

    $headers = ['cab','rak','sub','tipe','shlv','no_urut','jenis','plu','div','dep','kat','deskripsi','tag','tag_omi','pkmt','mplus','leadtime','sat','frc','stt','accost','lcost','lpp','lpp_rph','sls_qty_06','sls_qty_07','sls_qty_08','sls_rph_06','sls_rph_07','sls_rph_08','sls_hpp_06','sls_hpp_07','sls_hpp_08','top','tgl_aktif','tglbpb','tgl_ed','tgl_tag','namasupplier'];
    $sql = build_detail_sql($where_plu);
    ...
    foreach ($target_servers as $server) {
        ...
        while ($row=pg_fetch_assoc($result)) {
            ...
            foreach ($headers as $h) {
                ...
                $text_fields=['plu','div','dep','kat','rak','sub','shlv','no_urut','tag','tag_omi'];
...
if ($is_searched) {
    foreach ($target_servers as $server) {
        ...
        // Tetap tampilkan cabang walaupun hasilnya 0.
        $rekap_rows[] = [
...
// ---------------------------------------------------------
// 7. EXPORT EXCEL - JUGA REKAP RINGAN, BUKAN DETAIL
// ---------------------------------------------------------
if ($action === 'export_excel' && $is_searched) {
    $filename = 'Rekap_LPP_' . date('Ymd_His') . '.xls';
    header('Content-Type: application/vnd.ms-excel; charset=utf-8');
    header('Content-Disposition: attachment; filename="' . $filename . '"');
    header('Pragma: no-cache');
    header('Expires: 0');
    ...
    echo '</tr>';
    foreach ($rekap_rows as $r) {
        echo '<tr>';
        echo '<td>' . htmlspecialchars($r['KODE_CABANG']) . '</td>';
        echo '<td>' . htmlspecialchars($r['TIPE']) . '</td>';
        echo '<td style="text-align:right">' . number_format($r['TOTAL_ITEM'], 0, '.', '') . '</td>';
        echo '<td style="text-align:right">' . number_format($r['TOTAL_QTY'], 2, '.', '') . '</td>';
        echo '<td style="text-align:right">' . number_format($r['TOTAL_RUPIAH'], 2, '.', '') . '</td>';
        echo '</tr>';
    }
...
<title>Rekap LPP - Multi IP IGR SPI ICM</title>
<style>
...
<div class="header-container">
    <div class="top-row">
        <div>
            <h2>Monitoring & Rekap LPP Multi IP Database</h2>
            <small style="color:#64748B">Tampilan awal memakai query REKAP ringan. <b>Download DETAIL XLS</b> menjalankan query detail sesuai header query awal.</small>
        </div>
        <?php if ($is_searched): ?>
        <div class="btn-group">
    ...
    <form method="GET" action="" class="filter-form">
        ...
        <div class="form-group">
            ...
            <span class="help-text">Bisa dipisah koma, spasi, atau titik koma.</span>
        </div>
        <div class="form-group">
    ...
    <div class="note">PLU kosong = semua PLU. Jika ada error, error PostgreSQL akan ditampilkan per cabang/server di bawah.</div>
</div>
