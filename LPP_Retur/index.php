...
// Filter Dynamic PLU (PRD_PRDCD / ST_PRDCD)
...
// ---------------------------------------------------------
// 3. DAFTAR PILIHAN QUERY (LPP RETUR)
// ---------------------------------------------------------
$queries = [
    'lpp_retur' => [
        'title' => 'LPP RETUR',
...
$active_query_config = $queries['lpp_retur'];
$base_query = $active_query_config['sql'];
...
// ---------------------------------------------------------
// 4. PENANGANAN EXPORT (CSV, XLS, DAN XLSX)
// ---------------------------------------------------------
$action = isset($_GET['action']) ? $_GET['action'] : '';

if (in_array($action, ['export_csv', 'export_xls', 'export_xlsx'])) {
    $filename = "Laporan_LPP_RETUR_" . date('Ymd_His');
    ...
    if ($action === 'export_csv') {
        ...
        // Grand Total Row
        if ($is_header_printed) {
            $total_row = array_fill(0, 23, '');
            $total_row[0] = 'GRAND TOTAL';
            $total_row[22] = $grand_qty;
            $total_row[23] = $grand_rp;
                ...
                if ($result) {
                    ...
                    while ($row = pg_fetch_assoc($result)) {
                        ...
                        foreach ($row as $key => $value) {
                            ...
                            if (in_array($field_name, ['plu', 'div', 'dep', 'kat', 'rak', 'sub', 'shlv', 'no_urut'])) {
                                echo "<td style='mso-number-format:\"\\@\";'>" . $val . "</td>";
                            } elseif (in_array($field_name, ['lpp', 'lpp_rph', 'acost', 'lcost', 'pkmt', 'mplus', 'frc'])) {
    ...
    // --- C. EXPORT XLSX (Excel XML 2003 / Spreadsheet Format) ---
    if ($action === 'export_xlsx') {
            ...
            if ($dbconn) {
                ...
                if ($result) {
                    if (!$is_header_printed) {
                        ...
                        echo '<Cell><Data ss:Type="String">NAMA SERVER</Data></Cell>';
                        $num_fields = pg_num_fields($result);
                        for ($i = 0; $i < $num_fields; $i++) {
                            echo '<Cell><Data ss:Type="String">' . htmlspecialchars(strtoupper(pg_field_name($result, $i))) . '</Data></Cell>';
                        }
                        echo '</Row>';
                        $is_header_printed = true;
                    }
                    ...
                    while ($row = pg_fetch_assoc($result)) {
                        echo '<Row>';
                        echo '<Cell><Data ss:Type="String">' . htmlspecialchars($server['name']) . '</Data></Cell>';
                        foreach ($row as $key => $value) {
                            $val = htmlspecialchars($value ?? '');
                            $field_name = strtolower($key);

                            if (in_array($field_name, ['plu', 'div', 'dep', 'kat', 'rak', 'sub', 'shlv', 'no_urut'])) {
                                echo '<Cell ss:StyleID="TextFormat"><Data ss:Type="String">' . $val . '</Data></Cell>';
                            } elseif (in_array($field_name, ['lpp', 'lpp_rph', 'acost', 'lcost', 'pkmt', 'mplus', 'frc']) && is_numeric($value)) {
                                echo '<Cell ss:StyleID="NumFormat"><Data ss:Type="Number">' . (float)$value . '</Data></Cell>';
                            } else {
        ...
        if ($is_header_printed) {
            echo '<Row ss:StyleID="TotalStyle">';
            echo '<Cell><Data ss:Type="String">GRAND TOTAL</Data></Cell>';
            for ($c = 1; $c <= 21; $c++) echo '<Cell><Data ss:Type="String"></Data></Cell>';
            echo '<Cell ss:StyleID="NumFormat"><Data ss:Type="Number">' . $grand_qty . '</Data></Cell>';
            echo '<Cell ss:StyleID="NumFormat"><Data ss:Type="Number">' . $grand_rp . '</Data></Cell>';
            echo '</Row>';
        }
...
// ---------------------------------------------------------
// 5. PENARIKAN DATA TANPA MENAMPUNG BANYAK ROW DI MEMORI
// ---------------------------------------------------------
...
if ($is_searched) {
    foreach ($target_servers as $server) {
        ...
        if ($dbconn) {
            ...
            if ($result) {
                while ($row = pg_fetch_assoc($result)) {
                    // 1. Simpan baris hanya jika masuk ke indeks rentang pagination halaman saat ini
                    if ($total_all_records >= $offset_start && $total_all_records < $offset_end) {
                        $display_rows[] = array_merge(['SERVER_NAME' => $server['name']], $row);
                    }

                    // 2. Kalkulasi grand total & penambahan total record
                    $total_all_records++;
                    if (isset($row[$qty_col])) $grand_qty += (float)$row[$qty_col];
                    if (isset($row[$rp_col]))  $grand_rp += (float)$row[$rp_col];
                }
                pg_free_result($result);
            }
            pg_close($dbconn);
...
<head>
    ...
    <title>Monitoring LPP RETUR - Multi IP Database</title>
    <style>
        ...
        .btn { padding: 9px 14px; font-weight: bold; border-radius: 6px; text-decoration: none; border: none; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; font-size: 13px; transition: all 0.2s; }
        .btn-submit { background-color: #0F172A; color: white; margin-top: 18px; }
        .btn-submit:hover { background-color: #1E293B; }
        
        /* Export Buttons */
        .btn-xlsx { background-color: #107C41; color: white; }
        .btn-xlsx:hover { background-color: #0B5C30; }
        .btn-xls { background-color: #1D6F42; color: white; }
        .btn-xls:hover { background-color: #144E2E; }
...
<div class="header-container">
    <div class="top-row">
        ...
        <div class="btn-group">
            <?php if ($is_searched): ?>
                <a href="?<?= $query_string_base ?>&action=export_xlsx" class="btn btn-xlsx">
                    📥 Save .XLSX
                </a>
                <a href="?<?= $query_string_base ?>&action=export_xls" class="btn btn-xls">
                    📥 Save .XLS
                </a>
                <a href="?<?= $query_string_base ?>&action=export_csv" class="btn btn-csv">
                    📄 Save .CSV
                </a>
            <?php endif; ?>
        </div>
    ...
    <form method="GET" action="" class="filter-form">
        ...
        <div class="form-group">
            <label for="plu">Filter PLU (ST_PRDCD):</label>
            <input type="text" name="plu" id="plu" value="<?= htmlspecialchars($sel_plu) ?>" placeholder="Contoh: 0431090, 0431091">
            <span class="help-text">Pisahkan dengan koma jika banyak</span>
        </div>
                ...
                <?php foreach ($display_rows as $r): ?>
                    <tr>
                        <?php foreach ($r as $key => $value): 
                            ...
                            $val = htmlspecialchars($value ?? '');
                        ?>
                            <?php if (in_array($field_name, ['lpp', 'lpp_rph', 'acost', 'lcost', 'pkmt', 'mplus', 'frc'])): ?>
